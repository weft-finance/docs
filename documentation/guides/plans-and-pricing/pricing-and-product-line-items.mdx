---
title: 'Pricing & product line items'
description: 'Our pricing offers total flexibility.'
---
#### You can set your plans to be:
-   Fixed fees only   
-   Usage based only
-   Revenue Share based
-   Hybrid - a combination of usage and fixed fees

#### Adding tiers pricing
When creating a plan with pricing tiers, make sure to do the following:
1.  Click on **"Add products and prices"** and choose **"Tiers pricing items"**
2.  Click on **"Product source"** to select a product,
3.  Insert **name** and add a **description** (optional)
4.  **Start defining** the  quantities and prices in the **pricing** tiers table
5.  In the first row, insert the number that represents the final value within the range for the initial unit under the **"To"** column. Notice that in the subsequent row, the initial number under the **"From"** column will automatically adjust based on the number you've provided.
6.  Then click on **"Add tier"** until you reach the amount of units in the plan.
    
### Defining 'Grouped Products' (Bundle)
> You can group products to add variants of products and fees for a whole group of products. Commitment and base amount defined will apply on all variants associated with the group.
-   Start by creating a tiered plan, as explained above
-   Then click on the 3dot menu on the right side of the item
-   Select the 'product group' and name this group
-   This will enable "Product variant" selection and open up a new pricing table which you will have to fill out according to your plan
-   Click "Save" to keep your work