...
| Webhook Type | Description | Trigger Condition | Return Type |
|-------------|-------------|-------------------|-------------|
| Overage | Notifies when a customer exceeds their provisioned amount for a product | Customer's consumption exceeds their provisioned amount | JSON |
| Anonymous Customer Created | Notifies when a new anonymous customer is created in the system | Event was sent for a customer that couldn't exist | JSON |
...
you can read more about the product consumption type in the [product consumption](api-reference/pages/product-consumption) page.
